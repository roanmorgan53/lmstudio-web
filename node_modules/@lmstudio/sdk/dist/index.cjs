...
const llmStructuredPredictionTypeSchema = zod.z.enum(["none", "json"]);
...
const kvValueTypesLibrary = new KVFieldValueTypesLibraryBuilder({
    ...
    stringify: (value, _typeParam, { t }) => {
        switch (value) {
            ...
            case "favorMainGpu":
                return t("config:customInputs.llamaAccelerationSplitStrategy.favorMainGpu", "Favor Main GPU");
        }
    },
})
    ...
    schemaMaker: () => {
        ...
        const bitsSchema = zod.z
            .number()
            .int()
            .refine(n => validBits.has(n), n => ({ message: `Invalid bit value. Must be one of: 2, 3, 4, 6, 8. Got: ${n}` }));
        return zod.z.object({
            checked: zod.z.boolean(),
            value: bitsSchema,
        });
    ...
    schemaMaker: () => {
        ...
        const sizeSchema = zod.z
            .number()
            .int()
            .refine(n => validSizes.has(n), n => ({ message: `Invalid group size. Must be one of: 32, 64, 128. Got: ${n}` }));
        return zod.z.object({
            checked: zod.z.boolean(),
            value: sizeSchema,
        });
    ...
    schemaMaker: () => {
        return zod.z.object({
            checked: zod.z.boolean(),
            value: zod.z.number().int().nonnegative(),
        });
    },
    effectiveEquals: (a, b) => {
        if (a.checked !== b.checked) {
...
const globalConfigSchematics = new KVConfigSchematicsBuilder(kvValueTypesLibrary)
    ...
    .scope("mlx", builder => builder.field("topKSampling", "checkboxNumeric", { min: 1, max: 1000 }, { checked: false, value: 40 })))
    ...
    .scope("mlx", builder => builder
    .field("kvCacheBits", "mlxKvCacheBitsType", { isExperimental: true }, { checked: false, value: 8 })
    .field("kvCacheGroupSize", "mlxKvCacheGroupSizeType", { isExperimental: true }, { checked: false, value: 64 })
    .field("kvCacheQuantizationStart", "checkboxNumeric", { int: true, min: 0 }, { checked: false, value: 5000 })))
    .scope("llama.load", builder => builder
